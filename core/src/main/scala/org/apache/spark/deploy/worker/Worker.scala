...
private[deploy] class Worker(
  ...
  override def receive: PartialFunction[Any, Unit] = synchronized {
    ...
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          coresUsed += cores_
          memoryUsed += memory_
          sendToMaster((appId, execId, manager.state, None, None))
        } catch {
          case e: Exception =>
    ...
    case LaunchDriver(driverId, driverDesc) =>
      ...
      // 创建DriverRunner，封装了Driver启动所需要的资源等重要信息，内部实际上是ProcessBuilder（管理控制进程）
      val driver = new DriverRunner(
        ...
        workerUri,
        securityMgr)
      // 将这个Driver信息加入到Master的drivers（HashMap）内存结构中
      drivers(driverId) = driver
      // 启动driver这个ProcessBuilder内部的线程
      driver.start()
      // driver启动之后，将Worker用于启动Driver消耗的资源从记录到内存结构中
