 ...
 * spark功能的主要入口。一个sparkcontext代表对spark集群的连接，也可以用作创建RDDs，accumulators累加器
 * 和集群中的broadcast广播变量。
 *
 * Only one SparkContext may be active per JVM.  You must `stop()` the active SparkContext before
 * creating a new one.  This limitation may eventually be removed; see SPARK-2243 for more details.
 * 每个JVM中只能启动一个SparkContext。在创建一个新的sc之前，一定要`stop()`活动的sc。这个限制最终可能会被删除。
 *
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  try {
    ...
    // 创建SparkEnv（spark运行环境：etc：cache，map output tracher。。。）
  ...
  def runJob[T, U: ClassTag](
    ...
    // 在SparkContext初始化的时候创建了DAGScheduler，在这里进行stage划分等操作，最后提交taskSet来运行。
...
object SparkContext extends Logging {
   ...
   */
  // 在本地模式下执行Driver用的核心数；如果不是local模式，就是0
  private[spark] def numDriverCores(master: String): Int = {
    ...
    master match {
      ...
      case _ => 0 // driver is not used for execution Driver程序不用于执行。
    }
  }
