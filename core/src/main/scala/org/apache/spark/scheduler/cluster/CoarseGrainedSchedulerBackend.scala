  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    private def makeOffers() {
      /**
        * 第一步：执行任务分配算法，将各个task分配到executor上去
        * 第二步：将task分配完以后，执行launchTasks()方法，将分配的task通过消息LaunchTask发送到指定executor上去
        * 由executor执行对应的task。
        *
        * 给resourceOffers()传入的参数是Application所有可用的executor，并将其封装成了WorkerOffer对象，WorkerOffer对象中
        * 封装了每个executor可用的CPU的数量
        */
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      ...
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit >= maxRpcMessageSize) {
          ...
          // 给executor上的资源减去已经分配出去的资源
          executorData.freeCores -= scheduler.CPUS_PER_TASK

          logInfo(s"Launching task ${task.taskId} on executor id: ${task.executorId} hostname: " +
            s"${executorData.executorHost}.")
          // 向executor发送消息LaunchTask，在executor上启动task
  ...
  override def reviveOffers() {
    /**
      * driverEndpoint就是它自己的DriverEndpoint，他是在CoarseGrainedSchedulerBackend的start()中创建的
      * CoarseGrainedSchedulerBackend的start()是在StandaloneSchedulerBackend的start()方法中调用的
      * StandaloneSchedulerBackend的start()是在TaskSchedulerImpl的start()方法中调用的
      * 是在TaskSchedulerImpl的start()方法是在SparkContext中调用的
      *
      * 这里是给自己的消息体发消息ReviveOffers
       */
