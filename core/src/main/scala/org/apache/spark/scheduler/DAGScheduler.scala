...
class DAGScheduler(
   ...
   *
   * 包含了RDD的partitions的缓存位置。这个map的key是RDD的ids，value是partition位置的arrays。每个array的值都是RDD partition
   * 缓存的位置
   *
   * 每个调用该map的方法都必须是线程安全的
   */
  ...
  def getCacheLocs(rdd: RDD[_]): IndexedSeq[Seq[TaskLocation]] = cacheLocs.synchronized {
    ...
    // 如果这个rdd没有partition缓存下来，便将其partition缓存在cacheLocs中
   ...
   * 获得或创建shuffle map stage给给定RDD的map端。
   */
  ...
  private def newResultStage(
    ...
    // 用父stage，和stageId 创建自己的stage
    val stage = new ResultStage(id, rdd, func, partitions, parentStages, jobId, callSite)
    // 将resultstage放入内存结构stageIdToStage中
    stageIdToStage(id) = stage
   ...
   * recovered from the MapOutputTracker
    * 为给定RDD创建shuffle map rdd。这个stage将和给定的firstJobId关联。如果shuffleId对应的stage存在，说明
    * 这个shuffleId在MapOutputTracker中存在，就可以从MapOutputTracker中恢复这个RDDmap端产生的output的数量
    * 和位置。
   */
  private def newOrUsedShuffleStage(
    ...
    // 创建shuffleDep的map端为最后一个RDD的stage
    ...
    if (mapOutputTracker.containsShuffle(shuffleDep.shuffleId)) {
      ...
      // 将创建的shuffle map stage 注册到mapOutputTracker中。
   ...
   * the provided firstJobId.
    *
    * 得到或创建给定RDD的stages列表。这个新的stage会用提供的firstJobId创建。
   */
  private def getParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = {
    ...
    def visit(r: RDD[_]) {
      if (!visited(r)) {
        // 3. 如果visited结构中（访问过的RDD）不含r这个RDD，将r加入到visited中。
        visited += r
        // Kind of ugly: need to register RDDs with the cache here since
        // we can't do it in its constructor because # of partitions is unknown
        for (dep <- r.dependencies) {
          // 4. 判断r这个RDD和父RDD的依赖类型。
          dep match {
            case shufDep: ShuffleDependency[_, _, _] =>
              // 5. 如果r和父RDD的依赖关系是宽依赖，新建shuffleMapStage
    ...
    while (waitingForVisit.nonEmpty) {
      // 2. 调用visit(waitingForVisit中最上面的RDD)
      visit(waitingForVisit.pop())
    }
    // 返回finalRDD的所在stage的父stage。
    parents.toList
  }
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
      ...
      if (!visited(rdd)) {
        ...
        if (rddHasUncachedPartitions) {
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                // 如果这个RDD和父RDD是宽依赖，生成父RDD，存到missing结构中
                val mapStage = getShuffleMapStage(shufDep, stage.firstJobId)
                if (!mapStage.isAvailable) {
                  missing += mapStage
                }
              case narrowDep: NarrowDependency[_] =>
                // 如果是窄依赖，将父RDD继续执行visit()
    ...
    // 1. 将stage最后一个rdd压入栈waitingForVisit中
    waitingForVisit.push(stage.rdd)
    while (waitingForVisit.nonEmpty) {
      // 2. 将waitingForVisit中顶上rdd推出栈,调用def visit()
      visit(waitingForVisit.pop())
    }
    missing.toList
  }
   ...
   * Submit an action job to the scheduler.
    *  提交一个action job给scheduler
    *  rdd 目标RDD运行任务
    *  func 一个执行RDD每个partition的函数
    *  partitions 一组要运行的partitions；一些jobs不想运行目标RDD的所有partitions。eg.first()算子
  ...
  def submitJob[T, U](
    ...
    // 检查并确定我们没有在不存在的partition上运行task
  ...
  /**
    * DAGScheduler 调度作业，stage划分算法 的核心入口
    * stage的划分算法实在是太重要了，因为对于spark开发人员，
    * 必须对spark的划分算法非常的精通，知道自己的application 被划分成了几个job，
    * 每个job被划分成了几个stage，
    * 每个stage中是那些代码，
    * 只有明白了这些，才能在某个stage运行特别慢，或者是报错的时候，
    * 才能到对应代码去排查，或者性能调优。
    *
    * 总结：
    * 1. 从finalStage倒推
    * 2. 从宽依赖进行新的stage划分
    * 3. 使用递归算法得到所有的missstage，并且优先提交父stage。
    *
    */
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    // 使用触发job的最后一个rdd，来触发stage
    var finalStage: ResultStage = null
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      // 用finalRDD创建finalStage,并且将stage加入到DAGScheduler的内存缓存中
      // 在这个方法中会将finalRDD的所有祖先RDD加入到stageIdToStage内存结构中
    ...
    // 用finalStage创建一个Job，ActiveJob封装了Job的一些信息
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        // 寻找stage的父stage,并将父stage和所有的祖先stage加入到shuffleToMapStage内存结构中。
        val missing = getMissingParentStages(stage).sortBy(_.id)
        logDebug("missing: " + missing)
        if (missing.isEmpty) {
          // 如果stage不存在父stage了，那么就是第一个stage，开始提交task
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    stage match {
      case s: ShuffleMapStage =>
        /** 向authorizedCommittersByStage中加入记录
          * authorizedCommittersByStage = mutable.Map[StageId, Array[TaskAttemptNumber]]()
          * stageId
          * Array[TaskAttemptNumber] 一个数组，长度是stage最后一个rdd的partition数量
          * 数组首先将所有的值置为NO_AUTHORIZED_COMMITTER: TaskAttemptNumber = -1
          * */
    ...
    // 广播task的二进制代码，用于向executors分发tasks。注意，我们广播序列化之后的RDD副本，我们会为task反序列化该RDD，
    // 意味着每个task获得不一样的RDD副本。这提供了在闭包中修改引用对象的修改状态的tasks的隔离性。这对于Hadoop是非常必要的，
    // 因为JobConf/Configuration对象不是线程安全的。
    var taskBinary: Broadcast[Array[Byte]] = null
    try {
      // For ShuffleMapTask, serialize and broadcast (rdd, shuffleDep).
      // For ResultTask, serialize and broadcast (rdd, func).
      // 对于shuffleMapTask，序列化并广播(stage.rdd, shuffleDep)
      // 对于ResultTask，序列化病广播(stage.rdd,func)
    ...
    val tasks: Seq[Task[_]] = try {
      ...
      stage match {
        ...
        case stage: ResultStage =>
          // ResultStage创建对应的ResultTask，这里非常重要的是 最优位置算法
    ...
    if (tasks.size > 0) {
      ...
      /**
        * 对每个stage的task创建一个TaskSet，将TaskSet提交到TaskScheduler的submitTasks()去执行
       */
   ...
   * 获取与特定RDD分片的相关联location（位置）信息。
   *
   * This method is thread-safe and is called from both DAGScheduler and SparkContext.
   *
   * @param rdd whose partitions are to be looked at  该rdd的分片将被查看
   * @param partition to lookup locality information for  该partition的位置信息将被查看
   * @return list of machines that are preferred by the partition 该分区首选的计算列表
   */
  private[spark]
   ...
   * DAGScheduler state accessed by it may require additional synchronization.
    ...
    * 说白了就是从stage的最后一个rdd开始，去找哪个rdd的partition是被
    * cache/checkpoint了的，task优先task最佳位置就是cache/checkpoint的位置，
    * 因为这样的话，task直接在那个节点上执行而不用执行存储之前的rdd了
    *
   */
  private def getPreferredLocsInternal(
    ...
    // 注意：传入的visited:HashSet是一个空HashSet
    // If the partition has already been visited, no need to re-visit.
    // 如果该partition的位置信息已经找过了，就不用再寻找了
    // This avoids exponential path exploration.  SPARK-695
    if (!visited.add((rdd, partition))) {
      // Nil has already been returned for previously visited partitions.
      // 为什么返回Nil？HashSet已经有这个键值对，说明这个partition已经被访问过
      return Nil
    }
    // If the partition is cached, return the cache locations
    // 寻找当前rdd的partition是否被缓存了，如果该partition已经被缓存在本地，直接返回cache的位置
    // cache的位置比checkpoint的位置优先返回
    ...
    /**
      * 判断当前rdd的partition是否被checkpoint了，如果被checkpoint
      * 调用rdd的partitions(partition)方法，看rdd对该partition是否有最佳位置判断。如果有，返回rdd最佳
      * 位置判断
      * rdd.preferredLocations返回的数组中的String只会有三种格式：
      * executor_[hostname]_[executorid]，[hostname],hdfs_cache_[hostname]
      * 这三种格式会给TaskLocation处理判断数据存在哪里
     */
    ...
    // 递归调用:
    // 如果RDD是窄依赖，选择第一个有位置优选的窄依赖的第一个partition。
    // 理想情况下，我们会根据传输大小进行选择，但现在这样做。
    // 如果这个partition的父partition有位置优选，该partition的位置信息用父partition的位置信息。
    ...
    // 如果在这个stage中，从最后一个rdd到第一个rdd都没有被缓存或者被checkpoint，那么在DAGScheduler中就没有最佳
    // 位置，最佳位置需要TaskScheduler去分配
    Nil
  }
